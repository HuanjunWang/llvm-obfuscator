...
private:
	inline uint64_t uint8_to_64(uint8_t c) {
		uint64_t res =
			(((uint64_t)c) & 0xff) |
			(((uint64_t)c << 8) & 0xff00) |
			(((uint64_t)c << 16) & 0xff0000) |
			(((uint64_t)c << 24) & 0xff000000) |
			(((uint64_t)c << 32) & 0xff00000000) |
			(((uint64_t)c << 40) & 0xff0000000000) |
			(((uint64_t)c << 48) & 0xff000000000000) |
			(((uint64_t)c << 56) & 0xff00000000000000);
		return res;
    ...
    void addDecodeFunction(Module *mod, std::vector<encVar*> *gvars) {
        ...
        for (unsigned i = 0, e = gvars->size(); i != e; ++i) {
            ...
            uint64_t key = uint8_to_64((*gvars)[i]->key);
            ...
            // Decode
            ConstantInt* const_key = ConstantInt::get(mod->getContext(), APInt(8*cdata->getElementByteSize(), key));
            ...
            void_21->setAlignment(cdata->getElementByteSize());
